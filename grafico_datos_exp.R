...
grafico_datos_exp <- function(cost.data.pob, datos.pca, datos.indice, tabla.edos, nom.indice, num.corte, nom.abr, crs.proj.wgs, crs.proj.utm) {
...
susc.coords.proj <- st_transform(susc.coords.sf, crs.proj.utm) %>% # This trick update the factor levels
...
if(grepl("Exposicion",nom.indice)) cols <- c("forestgreen","gold","firebrick")
...
barplot.data <- susc.res %>% 
  ...
  mutate(NOM_ENT_fac=factor(NOM_ENT, levels=ent.ord),
         category_fac=factor(category, levels=c("Alta","Mediana","Baja")),
...
for(eachcategory in c("Baja", "Mediana", "Alta")){
  ...
  if(eachcategory=="Baja")thiscol = cols[1]
  if(eachcategory=="Mediana")thiscol = cols[2]
  if(eachcategory=="Alta")thiscol = cols[3]
