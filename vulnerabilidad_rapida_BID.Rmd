...
pobr.in <- cost.data.pob %>% 
  dplyr::select(COM_ID, COM_MUN, NOM_LOC,POBTOT,P15YM_SE,P_15YMAS,PDER_SEGP,PSINDER,VPH_SNBIEN,TVIVPARHAB,P_12YMAS,PDESOCUP) %>% 
  mutate(across(P15YM_SE:PDESOCUP, as.numeric)) %>% 
  mutate_if(is.numeric, funs(ifelse(is.na(.), 0, .))) %>% 
  mutate(prop_sin_escolar = P15YM_SE/P_15YMAS) %>% # + negativo
  mutate(prop_sin_salud = ((PDER_SEGP+PSINDER)/POBTOT)) %>% # + negativo 
...
pob.indig.in <- cost.data.pob %>% 
  ...
  mutate(prop_pind = 1-(as.numeric(PHOG_IND)/POBTOT)) %>% #+ negativo
...
wordclim.ssp126 <- stack(here("inputs","WorldClim","ssp126","wc2.1_2.5m_bioc_CNRM-CM6-1_ssp126_2041-2060.tif"))
...
wordclim.ssp585 <- stack(here("inputs","WorldClim","ssp585","wc2.1_2.5m_bioc_CNRM-CM6-1_ssp585_2041-2060.tif"))
...
for(eachlayer in 1:3){
  ...
  plot(this.data)
  writeRaster(this.data,paste0(here("inputs","exposure","futuro_terrestre/"),"_",these.names[eachlayer],"_2041-2060_ssp585"), format="GTiff", overwrite = TRUE)
...
exp.files <- list.files(path=here("inputs","exposure"),pattern = "*.tif$|*.shp$", recursive = TRUE, full.names = TRUE) %>% 
  as_tibble %>% 
...
expo.data.hist.fut <- expo.data.hist %>% 
  dplyr::select(-oxigeno_disuelto,-temperatura_superficial_del_mar,-productividad_primaria, -intervalo_diurno, -precipitacion_promedio,-temperatura_promedio_anual)
...
vul_analisis(expo_dat, adap_dat, susc_dat, escenario.nom, expo.base)
...
expo_dat  <- here("outputs","analysis","Exposición historica_datos_graficos.csv")
adap_dat <- here("outputs","analysis","adaptacion_datos_graficos.csv")
susc_dat <- here("outputs","analysis","susceptibilidad_datos_graficos.csv")
escenario.nom <- "Vulnerabilidad historica"
...
vul.ent.126 <- valores.vul.126 %>% 
  dplyr::group_by(region, NOM_ENT, escenario) %>% 
  dplyr::summarise(mean_cap = mean(adaptacion), 
                   sd_cap = sd(adaptacion),
                   n = n(),
                   se_cap = sd_cap / sqrt(n),
                   mean_expo = mean(delta_expo_norm), 
                   sd_expo = sd(delta_expo_norm),
                   se_expo = sd_expo / sqrt(n),
                   mean_susc = mean(susceptibilidad), 
                   sd_susc = sd(susceptibilidad),
                   se_susc = sd_susc / sqrt(n),
                   mean_vul = mean(vul_norm), 
                   sd_vul = sd(vul_norm),
                   se_vul = sd_vul / sqrt(n))

readr::write_csv(vul.ent.126, here::here("outputs","analysis","media_entidad_vul_126.csv"))

vul.ent.585 <- valores.vul.585 %>% 
  dplyr::group_by(region, NOM_ENT, escenario) %>% 
  dplyr::summarise(mean_cap = mean(adaptacion), 
                   sd_cap = sd(adaptacion),
                   n = n(),
...
readr::write_csv(vul.ent.585, here::here("outputs","analysis","media_entidad_vul_585.csv"))
