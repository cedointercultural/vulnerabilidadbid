...
```{r permisos_acuicola_pesca, echo=FALSE}
...
num.permisos.pesca.pac <- read_xlsx(here("inputs","sensitivity","permisos_pesca_comercial.xlsx"), sheet = 1) %>% 
  ...
  mutate(index_no=1) %>% 
  group_by(COM_MUN) %>% 
  summarise(num_permisos_pesca = sum(index_no)) 
...
num.permisos.pesca.gom.may <- read_xlsx(here("inputs","sensitivity","GOM-permisos_pesca-2016-2018.xlsx"), sheet = 1) %>% 
  ...
  group_by(COM_MUN) %>% 
  summarise(num_permisos_pesca = sum(num_permisos_pesca))
...
permisos.pesca.mun <- data.comm.costas %>% 
   ...
   mutate_if(is.numeric, funs(ifelse(is.na(.), 0, .))) %>% 
  mutate(permisos_pesca_per_capita_mun = num_permisos_pesca / tot_pob_mun) #permisos per capita
...
susc.datos.norm <- susc.datos.com %>%   
 dplyr::select(-ENTIDAD, -NOM_ENT, -MUN, -NOM_MUN, -LOC, -POBTOT, -COM_MUN, -pob_mun, -tot_pob_mun, -tot_captura_loc, -tot_captura_mun, -num_permisos_pesca, -num_permisos_acuicolas, -neg_pesca, -neg_acuicolas, -tot_ue) %>% 
    mutate_all(~replace(., is.na(.), 0)) %>% 
  mutate_if(is.numeric,~ scales::rescale(., to = c(0, 1)))

susc.datos.norm %>% write_csv(here("inputs","sensitivity","data_summary","datos_susceptibilidad_normalizados.csv"))
...
```{r capacidad_adaptacion, echo=FALSE}
...
comp.pob.in <- cost.data.pob %>% 
  dplyr::select(COM_ID, NOM_LOC, POBTOT,PHOGJEF_F,POCUPADA,P_12YMAS,GRAPROES,P18YM_PB,P_18YMAS,TOTHOG,POB0_14) %>% 
  mutate(across(PHOGJEF_F:POB0_14, as.numeric)) %>%
  mutate_if(is.numeric, funs(ifelse(is.na(.), 0, .))) %>% 
  mutate(prop_hogares_mujeres = PHOGJEF_F/TOTHOG) %>% 
  mutate(prop_ocupada = (POCUPADA/P_12YMAS)) %>% 
  mutate(grado_escolar = GRAPROES) %>%
  mutate(prop_educacion = (P18YM_PB/P_18YMAS)) %>% 
  mutate(prop_menor = (POB0_14/POBTOT)) %>% 
  dplyr::select(-PHOGJEF_F,-POCUPADA,-GRAPROES,-P18YM_PB,-TOTHOG,-POB0_14, -POBTOT) %>% 
  mutate_if(is.numeric,~ scales::rescale(., to = c(0, 1))) %>% 
  mutate(prop_hogares_mujeres_sc = 1-prop_hogares_mujeres) %>% 
  mutate(prop_menor_sc = 1-prop_menor)

# Pobreza
# Población sin escolaridad	Población de 15 años y más sin escolaridad	P15YM_SE
# Población con servicios de salud subsidiados	Población sin afiliación a servicios de salud +Población afiliada a servicios de salud en el Instituto de Salud para el Bienestar	PDER_SEGP + PSINDER
...
#Total de viviendas particulares	Viviendas particulares habitadas, deshabitadas y de uso temporal.	TVIVPAR
...
pobr.in <- cost.data.pob %>% 
  dplyr::select(COM_ID, COM_MUN, NOM_LOC,POBTOT,P15YM_SE,P_15YMAS,PDER_SEGP,PSINDER,VPH_SNBIEN,TVIVPAR,P_12YMAS,PDESOCUP) %>% 
  mutate(across(P15YM_SE:PDESOCUP, as.numeric)) %>% 
  mutate_if(is.numeric, funs(ifelse(is.na(.), 0, .))) %>% 
  mutate(prop_sin_escolar = P15YM_SE/P_15YMAS) %>% 
  mutate(prop_sin_salud = ((PDER_SEGP+PSINDER)/POBTOT)) %>% 
  mutate(prop_hog_sbienes = VPH_SNBIEN/TVIVPAR) %>%
  mutate(prop_desocup = (PDESOCUP/P_12YMAS)) %>% 
  left_join(irs.data, by="COM_ID") %>% 
  left_join(pob.mun.al, by="COM_MUN") %>% 
...
carac.hogares <- cost.data.pob %>% 
  dplyr::select(COM_ID, COM_MUN, NOM_LOC,POBTOT,VPH_2YMASD, VIVPAR_HAB, VPH_SINTIC, VPH_PISOTI, VPH_DRENAJ, VIVPAR_HAB,VPH_SNBIEN,VPH_C_ELEC) %>% 
  mutate(across(VPH_2YMASD:VPH_C_ELEC, as.numeric)) %>% 
  mutate_if(is.numeric, funs(ifelse(is.na(.), 0, .))) %>% 
  mutate(hogares_recamaras = VPH_2YMASD/VIVPAR_HAB) %>% 
  mutate(hogares_tecnologia = ((VIVPAR_HAB - VPH_SINTIC)/VIVPAR_HAB)) %>% 
  mutate(hogares_ptierra = 1-(VPH_PISOTI/VIVPAR_HAB)) %>%
  mutate(hogares_servicios = ((VIVPAR_HAB - VPH_SNBIEN)/VIVPAR_HAB)) %>% 
  mutate(hogares_eelectrica = (VPH_C_ELEC/VIVPAR_HAB)) %>% 
  mutate(hogares_drenaje = (VPH_DRENAJ/VIVPAR_HAB)) %>% 
...
infra.com <- cost.data.pob %>% 
  dplyr::select(COM_ID, COM_MUN, NOM_LOC,VIVPAR_HAB, VPH_AGUADV) %>% 
  mutate(across(VIVPAR_HAB:VPH_AGUADV, as.numeric)) %>% 
  mutate_if(is.numeric, funs(ifelse(is.na(.), 0, .))) %>% 
  mutate(hogares_aguaent = VPH_AGUADV/VIVPAR_HAB) %>% 
  left_join(long.caminos, by ="COM_MUN") %>% 
  left_join(dist.via, by ="COM_ID") %>% 
  left_join(acceso.pav, by = "COM_ID") %>% 
  left_join(disp.loc, by = "COM_ID") %>% 
  mutate(dist_via = 1-dist_via) %>% 
  mutate(distancia = 1 - distancia) %>% 
...
ind.mar <- read_xls(here("inputs","adaptive_capacity","IML_2020.xls"), sheet =2) %>% 
...
carac.empleo <- cost.data.pob %>% 
  dplyr::select(COM_ID, COM_MUN, NOM_LOC, POBTOT, PEA, POCUPADA, GRAPROES, PDER_SS, P_12YMAS) %>% 
  mutate(across(PEA:P_12YMAS, as.numeric)) %>% 
  mutate_if(is.numeric, funs(ifelse(is.na(.), 0, .))) %>% 
  mutate(pob_ecactiva = PEA/P_12YMAS) %>% 
  mutate(pob_empleada = POCUPADA/P_12YMAS) %>% 
  mutate(grad_esc = GRAPROES) %>%
  mutate(pob_salud = PDER_SS/POBTOT) %>% 
  left_join(comp.eco, by="COM_MUN") %>% 
  left_join(ind.mar, by="COM_ID") %>% 
  dplyr::select(COM_ID, NOM_LOC, pob_ecactiva,pob_empleada,grad_esc,pob_salud, in_compeco, in_marg) %>% 
  mutate_if(is.numeric,~ scales::rescale(., to = c(0, 1))) %>% 
  mutate(in_marg = 1-in_marg) 
...
# Cambio en los hogares femeninos	Hogares en viviendas particulares habitadas donde la persona de referencia es mujer. Se considera un hogar en cada vivienda particular. Incluye casa única en el terreno; departamento en edificio; vivienda en vecindad; vivienda en cuarto de azotea; local no construido para habitación; vivienda móvil; refugio o clase no especificada. 		HOGJEF_F

carac.empleo <- cost.data.pob %>% 
  dplyr::select(COM_ID, NOM_LOC, POBTOT, PEA, POCUPADA, GRAPROES, PDER_SS, P_12YMAS) %>% 
  mutate(across(PEA:P_12YMAS, as.numeric)) %>% 
  mutate_if(is.numeric, funs(ifelse(is.na(.), 0, .))) %>% 
  mutate(pob_ecactiva = PEA/P_12YMAS) %>% 
  mutate(pob_empleada = POCUPADA/P_12YMAS) %>% 
  mutate(grad_esc = GRAPROES) %>%
  mutate(pob_salud = PDER_SS/POBTOT) %>% 
  dplyr::select(COM_ID, NOM_LOC, pob_ecactiva,pob_empleada,grad_esc,pob_salud) %>% 
  mutate_if(is.numeric,~ scales::rescale(., to = c(0, 1))) 
...
eco.ind <- st_read(here("inputs","adaptive_capacity","ecoindigw.shp")) %>% 
  as_tibble %>% 
  mutate(prop_indes =  pidesocup/pobindi) %>% 
  na.omit %>% 
  mutate(COM_MUN = paste(as.numeric(CVE_ENT),as.numeric(CVE_MUN),sep="_")) %>% 
...
mig.ind <- st_read(here("inputs","adaptive_capacity","migindigw.shp")) %>% 
  as_tibble %>% 
  na.omit %>% 
  mutate(prop_inmig =  pilnacoten/pobindi) %>% 
...
pob.indig <- cost.data.pob %>% 
  dplyr::select(COM_ID, COM_MUN, NOM_LOC, POBTOT, PHOG_IND) %>% 
  mutate(prop_pind = as.numeric(PHOG_IND)/POBTOT) %>% 
  left_join(mig.ind, by="COM_MUN") %>% 
  left_join(ana.ind,by="COM_MUN") %>% 
  left_join(eco.ind,by="COM_MUN") %>% 
  mutate_if(is.numeric, funs(ifelse(is.na(.), 0, .))) %>% 
...
dis.personal <- cost.data.pob %>% 
  ...
  mutate(cam_pe = (pea/p_12ymas)-(PEA/P_12YMAS)) %>% #numero positivo es malo
  mutate(cam_mig = PRESOE15/P_5YMAS) %>% #mayor es malo
  mutate(cam_pobsol = (p12ym_solt/p_12ymas)-(P12YM_SOLT/P_12YMAS)) %>% #num. positivo es bueno
  mutate(cam_hogfem = (hogjef_f/pobtot)-(HOGJEF_F/POBTOT)) %>% #num. positivo es bueno
  dplyr::select(COM_ID, NOM_LOC, cam_pe, cam_mig, cam_pobsol, cam_hogfem) %>% 
  mutate_if(is.numeric,~ scales::rescale(., to = c(0, 1))) %>% 
  mutate(cam_pe = 1-cam_pe) 
...
adap.datos <- cost.data.pob %>% 
  dplyr::select(COM_ID, NOM_LOC, GRAPROES,PDER_SS,POBTOT,P_0A2,P_3A5, P_6A11,P_12A14, PHOG_IND) %>% 
...
#  mutate(por_salud = (PHOGIND/POBTOT)) %>%
...
sst.raster.data <- raster(here("inputs","exposure","Variables","Historico","current_sst_mean.tif"))

prod.raster.data <- raster(here("inputs","exposure","Variables","Historico","mean_cbpm_current.tif"))
...
"indice compuesto" de capacidad adaptativa por municipio conjuntando el atlas de cambio climatico del inecc y vulnerabilidad para comunidades costeras

https://atlasvulnerabilidad.inecc.gob.mx/page/index.html#

```{r}

vul.terrestre <- read_xlsx("Vulnerabilidad municipios mexico inecc_rev.xlsx", sheet = 1) %>% 
  mutate(MUN_ID = as.numeric(`Clave Municipio`),
         ENT_ID = as.numeric(`Clave Entidad`))

vul.list <-  list.files(pattern="vulne_datos*.*")

calc_vul_col <- function(vul_datos){
  
  print(vul_datos)
  
escenario.nom <- vul_datos %>% 
  str_split("_") %>% 
  unlist %>% 
  .[3] %>% 
  gsub(".csv","", .)

vul.costera <- read_csv(vul_datos) %>% 
separate(COM_ID,c("ENT_ID","MUN_ID","LOC_ID"),"_") %>% mutate(MUN_ID = as.numeric(MUN_ID)) %>% 
  mutate(ENT_ID = as.numeric(ENT_ID))
  

vul.tot <- vul.costera %>% 
  left_join(vul.terrestre, by=c("ENT_ID","MUN_ID")) %>% 
   mutate(vul_ganadera=as.numeric(`Vulnerabilidad de la producción ganadera extensiva ante estrés hídrico`)) %>% 
   mutate(vul_forrajera=as.numeric(`Vulnerabilidad de la producción forrajera ante estrés hídrico`)) %>% 
  mutate(vul_ganadera_inundaciones=as.numeric(`Vulnerabilidad de la producción ganadera extensiva a inundaciones`)) %>% 
  mutate(vul_deslaves=as.numeric(`Vulnerabilidad de asentamientos humanos a deslaves`)) %>% 
  mutate(vul_inundaciones=as.numeric(`Vulnerabilidad de asentamientos humanos a inundaciones`)) %>% 
  mutate(vul_dengue=as.numeric(`Vulnerabilidad de la población al incremento en distribución del dengue`)) 

vul.tot.edo <- vul.tot %>% 
  dplyr::group_by(NOM_ENT, ENT_ID) %>% 
  dplyr::summarise(edo_vul_ganadera=mean(vul_ganadera, na.rm = TRUE),
            edo_vul_forrajera=mean(vul_forrajera, na.rm = TRUE),
            edo_vul_ganadera_inundaciones=mean(vul_ganadera_inundaciones, na.rm = TRUE),
            edo_vul_deslaves=mean(vul_deslaves, na.rm = TRUE),
            edo_vul_inundaciones=mean(vul_inundaciones, na.rm = TRUE),
            edo_vul_dengue=mean(vul_dengue, na.rm = TRUE))
            
vul.comb.tot <- vul.tot %>% 
  dplyr::select(NOM_ENT, NOM_MUN, ENT_ID, MUN_ID, LOC_ID, dec_lon, dec_lat, NOM_LOC, region, vul_norm, vul_ganadera, vul_forrajera, vul_ganadera_inundaciones, vul_deslaves, vul_inundaciones, vul_dengue) %>%
  left_join(vul.tot.edo, by=c("NOM_ENT","ENT_ID")) %>% 
  mutate(vul_ganadera= if_else(is.na(vul_ganadera),edo_vul_ganadera,vul_ganadera),
         vul_forrajera= if_else(is.na(vul_forrajera),edo_vul_forrajera,vul_forrajera),
         vul_ganadera_inundaciones=if_else(is.na(vul_ganadera_inundaciones),edo_vul_ganadera_inundaciones, vul_ganadera_inundaciones),
         vul_deslaves=if_else(is.na(vul_deslaves),edo_vul_deslaves,vul_deslaves),
         vul_inundaciones=if_else(is.na(vul_inundaciones),edo_vul_inundaciones,vul_inundaciones),
         vul_dengue=if_else(is.na(vul_dengue),edo_vul_dengue, vul_dengue)) %>% 
mutate(vul_mean = rowMeans(dplyr::select(., c(vul_norm, vul_ganadera:vul_dengue)), na.rm = TRUE)) 
  

vul.comb.mun <- vul.comb.tot %>% 
  dplyr::group_by(NOM_ENT, NOM_MUN, region) %>% 
  dplyr::summarise(vul_mean_mun = (mean(vul_mean, na.rm = TRUE))) %>% 
  ungroup %>% 
  mutate(vul_mean_mun_norm = (vul_mean_mun-min(vul_mean_mun, na.rm = TRUE))/(max(vul_mean_mun, na.rm = TRUE)-min(vul_mean_mun, na.rm = TRUE)))


atl.vul <- vul.comb.mun %>% 
    arrange(vul_mean_mun_norm) %>% 
    top_n(10, vul_mean_mun_norm) %>% 
    arrange(desc(vul_mean_mun_norm)) 
  
loc.ord <- atl.vul  %>%  
    distinct(NOM_ENT, NOM_MUN, region) %>% 
    arrange(region, NOM_ENT, NOM_MUN) %>% 
    pull(NOM_MUN)
...
vul.loc.ord <-  atl.vul %>% 
    mutate(NOM_MUN_fac=factor(NOM_MUN, levels=loc.ord)) %>% 
...
#"#7A8C8E" Tabasco
#"#84ACB6" Tamaulipas
#"darkseagreen3" Nayarit
#"gray48"  
if(escenario.nom=="SSP126 2050") {
col.pal <- c("#CEDBE6", "#27CED7", "#335B74", "#3494BA","#58B6C0","#75BDA7", 
"#84ACB6")  } #BCS, Chiapas, Guerrero, Oaxaca, Sinaloa, Sonora, Tamaulipas
if(escenario.nom=="SSP126 2100") {
col.pal <- c("#CEDBE6", "#27CED7", "#335B74","darkseagreen3","#3494BA","#58B6C0","#75BDA7", 
"#84ACB6")  } #BCS, Chiapas,Guerrero, Nayarit, Oaxaca, Sinaloa, Sonora, Tamaulipas
if(escenario.nom=="SSP585 2050") {
  col.pal <- c("#CEDBE6","#373545","#27CED7","#335B74","#3494BA","#58B6C0","#75BDA7","#7A8C8E","#84ACB6")  } #BCS, Campeche, Chiapas, Guerrero, oaxaca, Sinaloa, Sonora, Tabasco, Tamaulipas
if(escenario.nom=="SSP585 2100") {
  col.pal <- c("#CEDBE6", "#27CED7", "#335B74", "#3494BA","#58B6C0", 
"#84ACB6", "gray48") } #BCS, Chiapas, Guerrero, Oaxaca, Sinaloa, Tamaulipas, veracruz
  

  alt.vul.plot <- vul.loc.ord %>% 
    ggplot()+
    geom_col(aes(y= vul_mean_mun_norm, x=NOM_MUN_fac, fill = NOM_ENT)) +
 scale_fill_manual(values=col.pal, name = "Entidad")+
   theme_light()+
   labs(y="Vulnerabilidad",
        x="Municipio",
        title = paste0("10 municipios más vulnerables ", escenario.nom))+
   scale_x_discrete(guide = guide_axis(angle = 90))

ggsave(paste0(escenario.nom,"_vul_comb.png"), alt.vul.plot, device="png", width = 6, height = 6)

}  

lapply(vul.list, calc_vul_col)
